(function () {
  ...
  function nudgePlayback(){
    ...
    let state;try{state=player.getPlayerState();}catch(_){return;}
    ...
    if(state===YT.PlayerState.UNSTARTED||state===YT.PlayerState.CUED||state===YT.PlayerState.BUFFERING){
      if(startupAttempts<5){startupAttempts++;try{player.playVideo();}catch(_){}startupTimer=setTimeout(nudgePlayback,startupAttempts===1?250:650);}
    }
  }
  ...
  function loadMedia(state){
    ...
    if(!playable){
      ...
      loadedKey=mediaKey;return;
    }
    els.stationCard.hidden=true;
    if(!playerReady)return;
    if(loadedKey!==mediaKey){
      loadedKey=mediaKey;loadedProgramVideoId=state.segment.videoId;startupPending=true;startupAttempts=0;
      try{player.unMute();player.setVolume(100);player.loadVideoById({videoId:state.segment.videoId,startSeconds:state.mediaSeconds});player.playVideo();}catch(_){}
      nudgePlayback();return;
    }
    if(startupPending)nudgePlayback();
    if(mode==="live"&&player.getPlayerState()===YT.PlayerState.PLAYING){
      const drift=state.mediaSeconds-player.getCurrentTime();if(Math.abs(drift)>2.5)player.seekTo(state.mediaSeconds,true);
    }
  }

  function tick(){
    const now=activeClockMs();ensureSchedule(now);
    ...
    renderNext(liveState.block);loadMedia(state);
  }
  ...
  function localShareCredit(reference){
    const attemptId=`nbc-share-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`;
    const parse=(key,fallback)=>{try{return JSON.parse(localStorage.getItem(key))||fallback;}catch(_){return fallback;}};
    const session=parse("starquest_session",null),users=parse("starquest_users",{}),signedIn=session&&session.key&&users[session.key];
  ...
  window.onYouTubeIframeAPIReady=function(){
    player=new YT.Player("player",{width:"100%",height:"100%",playerVars:{playsinline:1,controls:1,enablejsapi:1,autoplay:0,origin:location.origin,widget_referrer:location.href},events:{
      ...
      onError:()=>{if(loadedProgramVideoId)failedVideoIds.add(loadedProgramVideoId);startupPending=false;clearStartupTimer();scheduleKey="";loadedKey="";loadedProgramVideoId="";setTimeout(tick,150);}
    }});
  };

  els.enter.addEventListener("click",enterStation);els.startOver.addEventListener("click",startOver);els.rewind.addEventListener("click",rewind);els.live.addEventListener("click",joinLive);els.share.addEventListener("click",shareChannel);
  ensureSchedule(Date.now());tick();loadYouTubeApi();setInterval(tick,1000);
})();
