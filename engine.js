(function (root) {
  ...
  // One station clock for every viewer. Display code may show local times,
  // but schedule selection and playback position are anchored to Central time.
  ...
  function createSegments(block) {
    ...
    if (runtime < block.blockSeconds) {
      segments.push({
        kind: "station",
        title: "NBC station break — next program starts on schedule",
        videoId: "",
        cleared: true,
        sourceStart: 0,
